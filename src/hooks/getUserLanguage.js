...
export const getUserLang = ({ availableLanguages, selectedLanguage }) => {
  ...
  if (typeof window !== "undefined") {
    ...
    if (selectedLanguage && availableLanguages.includes(selectedLanguage)) return selectedLanguage
    else if (selectedLanguage && !availableLanguages.includes(selectedLanguage)) return availableLanguages[0]
      ...
      const alertedAboutLanguage = localStorage.getItem("alertedAboutLanguage") ?? false
      if (alertedAboutLanguage === false) {
        alert(`Did not find your user language ("${userLang}"). Setting default language: "${language}"`)
